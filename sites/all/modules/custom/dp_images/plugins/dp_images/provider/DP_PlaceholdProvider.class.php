 ...
 */
class DP_PlaceholdProvider extends PlaceholdProvider {
  ...
  function settingsForm() {
    ...
    $form['devel_image_provider_' . $this->plugin['name']]['devel_image_provider_width'] = array(
      '#type' => 'textfield',
      '#title' => t('Choose the width of the Image'),
      '#default_value' => isset($this->settings['devel_image_provider_width']) ? $this->settings['devel_image_provider_width'] : '960' ,
    );

    $form['devel_image_provider_' . $this->plugin['name']]['devel_image_provider_height'] = array(
      '#type' => 'textfield',
      '#title' => t('Choose the height of the Image'),
      '#default_value' => isset($this->settings['devel_image_provider_height']) ? $this->settings['devel_image_provider_height'] : '700' ,
    );    
  ...
  public function generateImage($object, $field, $instance, $bundle) {
    ...
    if (!isset($images[$extension][$min_resolution][$max_resolution]) || count($images[$extension][$min_resolution][$max_resolution]) <= DEVEL_GENERATE_IMAGE_MAX) {
      if ($tmp_file = drupal_tempnam('temporary://', 'imagefield_')) {
        ...
        $width = isset($this->settings['devel_image_provider_width']) ? $this->settings['devel_image_provider_width'] :rand((int) $min[0], (int) $max[0]);
        $height = isset($this->settings['devel_image_provider_height']) ? $this->settings['devel_image_provider_height'] :rand((int) $min[1], (int) $max[1]);
